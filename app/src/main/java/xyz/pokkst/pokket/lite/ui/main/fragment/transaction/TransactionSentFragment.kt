package xyz.pokkst.pokket.lite.ui.main.fragment.transaction
...
import xyz.pokkst.pokket.lite.util.BalanceFormatter
import xyz.pokkst.pokket.lite.util.ClipboardHelper
import xyz.pokkst.pokket.lite.util.PriceHelper
    ...
    override fun onCreateView(
        ...
        for (x in tx.outputs.indices) {
            if (ScriptPattern.isOpReturn(tx.outputs[x].scriptPubKey)) {
                ...
                val address =
                    tx.outputs[x].scriptPubKey.getToAddress(WalletManager.parameters).toString()
        ...
        val bchSent = -tx.getValueSentFromMe(WalletManager.wallet).toPlainString().toDouble()
    ...
    private fun setSentToAddresses(
        ...
        for (i in addresses.indices) {
            ...
            if (addresses[i] != null && addresses[i]!!.isNotEmpty()) {
                ...
                val toExchangeText = if (utxoIsMine) {
                    "($${BalanceFormatter.formatBalance(amountInFiat, "0.00")})"
                } else {
                    "($-${BalanceFormatter.formatBalance(amountInFiat, "0.00")})"
                }
                txToExchange.text = toExchangeText
